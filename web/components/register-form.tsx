...
import { RegistrationFormValues, RegistrationRole, hasValidationErrors, validateRegistrationForm } from '@/lib/validation';
...
export default function RegisterForm() {
  ...
  function updateField<K extends keyof RegistrationFormValues>(key: K, value: RegistrationFormValues[K]) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      fullName: isBusiness ? values.businessName.trim() : values.fullName.trim(),
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const mainMessage = data.message || fieldErrorMessages || 'Please check your details and try again.';
        setApiMessage({
          type: 'error',
      ...
      <section className="mx-auto grid min-h-[calc(100vh-3rem)] w-full max-w-6xl grid-cols-1 overflow-hidden rounded-[10px] border border-border-muted bg-transparent md:grid-cols-1 lg:grid-cols-2">
        <aside className="relative flex flex-col justify-center border-b border-border-muted p-6 sm:p-8 lg:border-b-0 lg:border-r lg:p-12">
          ...
          <div className="relative">
            ...
            <p className="mt-4 max-w-xl text-base leading-relaxed text-slate-300">
              PawnScan empowers individuals and businesses to verify high-value items instantly, preventing the
              circulation of stolen goods.
            </p>
            <div className="mt-6 inline-flex min-h-12 items-center gap-3 rounded-[10px] border border-border-muted bg-slate-950/70 px-4 py-3 text-slate-200">
              ...
              <p className="text-sm">Trusted verification for safer transactions.</p>
            </div>
          </div>
        </aside>
        ...
        <section className="flex items-center p-4 sm:p-8 lg:p-12">
          <div className="w-full rounded-[10px] border border-border-muted bg-slate-950/70 p-6 sm:p-8">
            ...
            <p className="mt-2 text-sm text-slate-300">Join the PawnScan network in a few simple steps</p>

            <form className="mt-6 space-y-4" onSubmit={handleSubmit} noValidate aria-label="Registration form">
              <fieldset>
                <legend className="mb-2 block text-sm font-medium text-slate-200">Account Type</legend>
                <div className="relative grid min-h-12 grid-cols-2 rounded-[10px] border border-border-muted p-1">
                  <span
                    aria-hidden="true"
                    className={`absolute bottom-1 top-1 z-0 w-[calc(50%-4px)] rounded-[8px] transition-transform duration-300 ${
                      values.role === 'BUSINESS' ? 'bg-brand/25' : 'bg-slate-500/35'
                    } ${
                      values.role === 'BUSINESS' ? 'translate-x-[calc(100%+4px)]' : 'translate-x-0'
                    }`}
                  />
                  <button
                    type="button"
                    ...
                    className={`relative z-10 flex min-h-12 items-center justify-center gap-2 rounded-[8px] px-4 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand ${
                      values.role === 'INDIVIDUAL' ? 'text-slate-50' : 'text-slate-300 hover:text-slate-100'
                    }`}
                  >
                    <span
                      aria-hidden="true"
                      className={`h-2.5 w-2.5 rounded-full ${
                        values.role === 'INDIVIDUAL' ? 'bg-slate-100' : 'bg-slate-500'
                      }`}
                    />
                    Individual
                  </button>
                  <button
                    ...
                    className={`relative z-10 flex min-h-12 items-center justify-center gap-2 rounded-[8px] px-4 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand ${
                      values.role === 'BUSINESS' ? 'text-slate-50' : 'text-slate-300 hover:text-slate-100'
                    }`}
                  >
                    <span
                      aria-hidden="true"
                      className={`h-2.5 w-2.5 rounded-full ${
                        values.role === 'BUSINESS' ? 'bg-brand' : 'bg-slate-500'
                      }`}
                    />
                    Business
                  </button>
              ...
              {!isBusiness && (
                <div>
                  ...
                  <input
                    ...
                    autoComplete="name"
                  />
                  {(submitAttempted || values.fullName.length > 0) && errors.fullName && (
                    <p className="mt-1 text-sm text-status-stolen">{errors.fullName}</p>
                  )}
                </div>
              )}
              ...
              {isBusiness && (
                <>
                  <div>
                    ...
                    />
                    {(submitAttempted || values.businessName.length > 0) && errors.businessName && (
                      <p className="mt-1 text-sm text-status-stolen">{errors.businessName}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {(submitAttempted || values.businessAddress.length > 0) && errors.businessAddress && (
                      <p className="mt-1 text-sm text-status-stolen">{errors.businessAddress}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="min-h-12 w-full rounded-[10px] border border-border-muted bg-slate-900 px-3 py-2 text-slate-100 transition focus-visible:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/30"
                    />
                    {(submitAttempted || values.permitNumber.length > 0) && errors.permitNumber && (
                      <p className="mt-1 text-sm text-status-stolen">{errors.permitNumber}</p>
                    )}
                  </div>
                </>
              )}
              ...
              <div>
                ...
                />
                {(submitAttempted || values.contactNumber.length > 0) && errors.contactNumber && (
                  <p className="mt-1 text-sm text-status-stolen">{errors.contactNumber}</p>
                )}
              </div>
              ...
              <div>
                ...
                {(submitAttempted || values.confirmPassword.length > 0) && errors.confirmPassword && (
                  <p className="mt-1 text-sm text-status-stolen">{errors.confirmPassword}</p>
                )}
              </div>
