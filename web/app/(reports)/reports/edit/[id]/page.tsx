...
export default function EditReportPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        message: "Report updated successfully. Redirecting to My Reports...",
      });
      ...
      <main className="mx-auto w-full max-w-5xl px-4 pb-16 pt-36 sm:px-6 sm:pt-40 md:pt-32 lg:px-8">
        <section className="glass-panel rounded-2xl bg-slate-900/35 p-6 shadow-[0_18px_36px_rgba(0,0,0,0.28)] sm:p-8">
          ...
          <h1 className="mt-2 text-2xl font-bold text-white sm:text-3xl">
            Update stolen item report
          </h1>

          {isLoading ? (
            <div className="mt-6 rounded-lg bg-slate-900/55 p-4 text-sm text-slate-300">
              Loading report...
            </div>
          ) : (
            <form
            ...
            >
              ...
              <label className="flex flex-col gap-2 text-sm">
                <span className="text-slate-300">
                  Replace or Add Attachment (optional)
                </span>
                <input
                  type="file"
                  accept="image/*,.pdf"
              ...
              {feedback && (
                <div
                  className={`rounded-lg border px-4 py-3 text-sm ${feedback.type === "error" ? "border-status-stolen/40 bg-status-stolen/10 text-status-stolen" : "border-status-clean/40 bg-status-clean/10 text-status-clean"}`}
                >
                  {feedback.message}
                </div>
              )}
              ...
              <div className="mt-2 flex flex-wrap gap-3">
                <button
                  type="submit"
                  disabled={!canSubmit}
                  className="w-full rounded-full bg-brand px-5 py-2.5 text-sm font-semibold text-bg-main transition hover:brightness-90 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
                >
                  {isSubmitting ? "Saving..." : "Save Changes"}
                </button>
                <Link
                  href="/reports"
                  className="w-full rounded-full bg-slate-800/65 px-5 py-2.5 text-center text-sm font-semibold text-slate-200 transition hover:bg-slate-700/75 sm:w-auto"
                >
                  Cancel
                </Link>
              </div>
